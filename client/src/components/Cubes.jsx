...
const Cubes = ({
  ...
  useEffect(() => {
    ...
    // Use document-level listeners but check if pointer is over the scene
    const handlePointerMove = (e) => {
      if (!sceneRef.current) return;
      const rect = sceneRef.current.getBoundingClientRect();
      // Only process if pointer is within scene bounds
      if (e.clientX >= rect.left && e.clientX <= rect.right && 
          e.clientY >= rect.top && e.clientY <= rect.bottom) {
        onPointerMove(e);
      }
    ...
    const handleClick = (e) => {
      if (sceneRef.current && sceneRef.current.contains(e.target)) {
        onClick(e);
      }
    };

    // Use document listeners to capture events even when z-index is negative
    document.addEventListener('mousemove', handlePointerMove, { passive: true });
    document.addEventListener('mouseleave', handlePointerLeave, { passive: true });
    document.addEventListener('click', handleClick, { passive: true });

    // Also keep element listeners as fallback
